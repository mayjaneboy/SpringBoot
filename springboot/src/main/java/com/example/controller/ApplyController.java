...
public class ApplyController {
    ...
    public Result update(@RequestBody Apply apply) {  //@RequestBody用来接受前端传来的json参数，（get请求是接收不倒json的）
        Account currentUser = TokenUtils.getCurrentUser();
    ...
    @GetMapping("/selectAll")  // 完整请求路径：  /apply/selectAll
    ...
    public Result selectPage(@RequestParam(defaultValue = "1") Integer pageNum,
                             ...
                             Apply apply) {
        //参数列表里的Apply apply可以接受多个 Apply对象的属性 作为参数，且不和前面的参数冲突
        PageInfo<Apply> pageInfo = applyService.selectPage(pageNum, pageSize, apply);
        //pageInfo是数据类型为实体类Apply的对象
        return Result.success(pageInfo);
        //pageInfo作为参数传给Result.success（），将pageInfo赋给Result对象的data，得到的是Result类型的对象，对应Apply.vue里的res
        //res.data是Apply类型的，但为什么res.data会有res.data.list,Apply类型的对象没有list属性啊
    }
}